      ...
      if (url.includes("/micro/content/cgi-bin/mmfinderassistant-bin/post/post_list")) {
        for (const row of list) {
          ...
          const item: VideoAccountMappedPost = existing ?? {
            id,
            title: postTitle(row) || `视频号作品 ${safeId(objectId).slice(-8)}`,
            publishedAt: toIsoDate(row.createTime),
            capturedAt,
            views: 0,
            likes: 0,
            comments: 0,
          ...
          item.title = postTitle(row) || item.title;
          item.publishedAt = toIsoDate(row.createTime) ?? item.publishedAt;
