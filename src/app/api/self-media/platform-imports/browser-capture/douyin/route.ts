...
const DOUYIN_CREATOR_URL = "https://creator.douyin.com/creator-micro/content/manage";
...
async function openSession() {
  ...
  await page.goto(DOUYIN_CREATOR_URL, { waitUntil: "domcontentloaded", timeout: 60000 }).catch(() => undefined);
  const session = { browser, context, page, openedAt: new Date().toISOString(), lastRows: [] };
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "close") {
      await closeSession();
      return Response.json(emptyResult(action, { ok: true, loginState: "closed", browserOpened: false, message: "已关闭抖音临时浏览器会话；未保存登录态。" }));
    }

    const session = action === "open" ? await openSession() : globalThis.__selfMediaDouyinBrowserSession;
    if (!session || session.page.isClosed()) {
      return Response.json(emptyResult(action, { message: "请先打开抖音后台临时浏览器会话。" }), { status: 400 });
    }
    ...
    if (action === "open" || action === "status") {
      return Response.json(emptyResult(action, {
        ...
        message: loginState === "needs_login" ? "请在弹出的抖音后台完成登录，再回到本页确认。" : "抖音临时浏览器会话已连接；可以进入作品管理/数据页面后抓取可见作品。"
      }));
    }
