...
readiness 脚本只生成 ready/blocked 报告和可派发 prompt，不调用 `create_thread` 或 `send_message_to_thread`。
...
- 新的独立 Worker：主会话/Ops 用 `create_thread`，线程名建议 `Worker <编号> - <短标题>`。
- 已有 Worker 继续同一任务：主会话/Ops 用 `send_message_to_thread`。
- `needs_user_gate=true`、`waiting_user_gate`、真实保存未确认、登录/扫码/验证码、删除、force push、敏感材料或重型 gate 冲突时，不派发。
