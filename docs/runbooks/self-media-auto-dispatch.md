...
- Use `create_thread` only when the user or Orchestrator explicitly wants a separate Worker thread.
- Use `send_message_to_thread` only for an already selected thread.
