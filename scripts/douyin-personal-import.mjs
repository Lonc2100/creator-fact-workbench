...
import { DouyinPersonalProvider } from "../src/domain/self-media/providers/douyin-personal-provider.ts";
import { importDouyinPersonalCaptures } from "../src/domain/self-media/runtime/self-media-runtime.ts";

const DEFAULT_RAW_DIR = path.join(process.cwd(), ".local", "douyin-personal-v0", "raw");
const DEFAULT_OUT_DIR = path.join(process.cwd(), ".local", "douyin-personal-v1");
...
const preview = {
  ...
  importResult: options.save ? await importDouyinPersonalCaptures(captures) : undefined
};
