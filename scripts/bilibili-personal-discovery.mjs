...
const TARGET_URL = "https://member.bilibili.com/creator/home";
const DEFAULT_CDP = "http://127.0.0.1:9222";
const OUTPUT_DIR = path.join(process.cwd(), ".local", "bilibili-personal-v0");
const RAW_DIR = path.join(OUTPUT_DIR, "raw");
const SENSITIVE_KEY =
  /(cookie|token|csrf|xsrf|session|secret|password|passwd|authorization|auth|ticket|captcha|verify|passport|mobile|phone|email|openid|user_id|userid|uid|mid|dedeuserid|bili_jct|sessdata|sid|buvid|uuid|fingerprint|access_key|avatar|face|nickname|uname|realname|signature|sign|login|qrcode|encrypt|key)/i;
const LONG_SECRET = /^[A-Za-z0-9+/=_-]{80,}$/;

const FIELD_TARGETS = [
  { key: "accountOverview", label: "账号总览", pattern: /(overview|summary|account|author|profile|user|dashboard|core|home|creator|member|up|stat|账号|主页|总览|概览|创作)/i },
  { key: "worksList", label: "作品 / 视频数据列表", pattern: /(archive|archives|aid|bvid|video|videos|work|works|content|material|article|opus|list|items|作品|视频|内容|稿件|专栏)/i },
  { key: "views", label: "播放 / 阅读", pattern: /(play|plays|view|views|read|reads|watch|vv|pv|uv|impression|exposure|show|click|播放|浏览|阅读|观看|曝光)/i },
  { key: "likes", label: "点赞", pattern: /(like|likes|liked|thumb|praise|点赞)/i },
  { key: "comments", label: "评论数", pattern: /(comment|comments|reply|replies|danmu|dm|barrage|评论|回复|弹幕)/i },
  { key: "saves", label: "收藏", pattern: /(collect|collections|collected|favorite|favorites|fav|save|saves|收藏)/i },
  { key: "shares", label: "分享 / 转发", pattern: /(share|shares|forward|repost|relay|retweet|分享|转发)/i },
  { key: "followersDelta", label: "粉丝变化", pattern: /(fans|fan|follower|followers|follow|new_fans|increase|increment|delta|change|粉丝|关注|增量|新增)/i },
  { key: "commentContent", label: "评论内容", pattern: /(comment.*content|comment.*text|reply.*content|reply.*text|message|content|text|评论内容|回复内容)/i }
];

function parseArgs(argv) {
  const options = {
    target: TARGET_URL,
    cdp: process.env.BILIBILI_CDP_ENDPOINT ?? DEFAULT_CDP,
    durationMs: Number(process.env.BILIBILI_DISCOVERY_DURATION_MS ?? 60000),
    maxCaptures: Number(process.env.BILIBILI_DISCOVERY_MAX_CAPTURES ?? 80),
    maxArrayItems: Number(process.env.BILIBILI_DISCOVERY_MAX_ARRAY_ITEMS ?? 80),
    noLaunch: false
  };
...
function findChromePath() {
  const candidates = [
    process.env.BILIBILI_CHROME_PATH,
...
async function launchLocalBrowser(options) {
  ...
  if (!chromePath) return { launched: false, reason: "No Chrome or Edge executable found. Set BILIBILI_CHROME_PATH or start Chrome with remote debugging." };
  const userDataDir = path.join(OUTPUT_DIR, "chrome-profile");
  mkdirSync(userDataDir, { recursive: true });
  const port = portFromCdp(options.cdp);
  const child = spawn(
    chromePath,
    [`--remote-debugging-port=${port}`, `--user-data-dir=${userDataDir}`, "--no-first-run", "--no-default-browser-check", options.target],
...
function isBilibiliCreatorUrl(value) {
  try {
    const hostname = new URL(value).hostname;
    return (
      hostname === "member.bilibili.com" ||
      hostname.endsWith(".member.bilibili.com") ||
      hostname === "api.bilibili.com" ||
      hostname.endsWith(".api.bilibili.com") ||
...
async function inferLoginState(page, captureCount) {
  if (!page || page.isClosed()) return captureCount > 0 ? "logged_in_or_accessible" : "unknown_no_json_captured";
  try {
    const currentUrl = page.url();
    const bodyText = await page.locator("body").innerText({ timeout: 3000 }).catch(() => "");
    if (/login|passport|sso|account\.bilibili\.com/i.test(currentUrl) || /登录|扫码登录|请先登录|验证码|安全验证|请完成验证|\bLog in\b/i.test(bodyText)) {
      return captureCount > 0 ? "maybe_logged_in_with_login_prompt" : "needs_login";
    }
...
function writeEndpoints(endpoints) {
  const output = [...endpoints.values()].map((endpoint) => ({
    ...
    candidateFields: Object.fromEntries(
      Object.entries(endpoint.candidateFields).map(([fieldKey, values]) => [
        fieldKey,
        values.map((value) => ({
          path: value.path,
          types: value.types
...
function coverageFromEndpoints(endpoints) {
  return Object.fromEntries(
    FIELD_TARGETS.map((target) => {
      const matches = endpoints.flatMap((endpoint) => endpoint.candidateFields[target.key] ?? []);
      return [target.key, { label: target.label, status: matches.length > 0 ? "candidate_paths_found" : "not_confirmed", candidatePathCount: matches.length }];
    })
  );
}

function writeCaptureSummary({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints, notes }) {
  const output = {
    generatedAt,
    ...
    cdpEndpoint,
    loginState,
    jsonCaptures: captures.length,
    endpointCount: endpoints.length,
    outputDir: ".local/bilibili-personal-v0",
    rawDir: ".local/bilibili-personal-v0/raw",
    report: ".local/bilibili-personal-v0/field-report.md",
    endpointSummary: endpoints.map((endpoint) => ({
      id: endpoint.id,
    ...
    coverage: coverageFromEndpoints(endpoints),
    notes
  };
  writeFileSync(path.join(OUTPUT_DIR, "capture-summary.json"), JSON.stringify(output, null, 2));
  return output;
}
...
function writeFieldReport({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints, notes }) {
  ...
  const lines = [
    "# Bilibili Personal V0 Field Discovery Report",
    "",
    ...
    "- Raw captures are sanitized and saved only under `.local/bilibili-personal-v0/raw/`.",
    "- This collector only observes the user's own logged-in Bilibili creator center pages.",
    "- It does not bypass CAPTCHA, risk control, login challenges, or collect public content in bulk.",
    "- It does not write durable Repo / Service / Runtime / MetricSnapshot records.",
    "",
    "## Notes",
    "",
    ...notes.map((item) => `- ${item}`),
  ...
  if (endpoints.length === 0) {
    lines.push("- No Bilibili creator-center JSON endpoints captured.");
  } else {
...
function writeReports({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints, notes }) {
  const endpointList = writeEndpoints(endpoints);
  writeFieldReport({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints: endpointList, notes });
  const summary = writeCaptureSummary({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints: endpointList, notes });
  return { endpointList, summary };
}
...
async function main() {
  ...
  if (!(await canReachCdp(options.cdp))) {
    if (options.noLaunch) {
      ...
      writeReports({ generatedAt: new Date().toISOString(), loginState: "not_connected", targetUrl: options.target, cdpEndpoint: options.cdp, captures, endpoints, notes });
  ...
  const attachPage = (page) => {
    ...
    page.on("response", async (response) => {
      try {
        ...
        if (!isBilibiliCreatorUrl(url)) return;
        const contentType = response.headers()["content-type"] ?? "";
        if (!/json/i.test(contentType) && !/\/(x|x2|api|creative|creator|data|archive|article|member|web|stat|up|reply)\//i.test(url)) return;
        ...
        // Continue discovery even if one response body is unavailable.
      }
    });
  };
  context.pages().forEach(attachPage);
  context.on("page", attachPage);
  let page = context.pages().find((item) => item.url().includes("member.bilibili.com") || item.url().includes("bilibili.com")) ?? context.pages()[0];
  if (!page) page = await context.newPage();
  attachPage(page);
  console.log("Bilibili discovery collector is running. If the page asks for login or verification, complete it manually in the browser. The script will not bypass challenges.");
  ...
  await sleep(Math.max(1000, options.durationMs));

  const currentPage = context.pages().find((item) => !item.isClosed() && (item.url().includes("member.bilibili.com") || item.url().includes("bilibili.com"))) ?? (!page.isClosed() ? page : undefined);
  if (page.isClosed()) notes.push("The original Bilibili page was closed or replaced during discovery; login state was inferred from the current browser pages and captured JSON.");
  const loginState = await inferLoginState(currentPage, captures.length);
  const hasLoginPrompt = loginState === "needs_login" || loginState === "maybe_logged_in_with_login_prompt";
  if (hasLoginPrompt) notes.push("The page appears to require login or still shows a login prompt. Log in manually and rerun the collector; login-page JSON is not treated as data discovery success.");
  if (captures.length === 0) notes.push("No Bilibili creator-center JSON responses were captured during the discovery window. Try logging in, refreshing the creator center, or clicking overview/content/data/fans/comment modules while the collector is running.");
  const { endpointList } = writeReports({ generatedAt: new Date().toISOString(), loginState, targetUrl: options.target, cdpEndpoint: options.cdp, captures, endpoints, notes });
  if (typeof browser.disconnect === "function") browser.disconnect();
  else await browser.close();
  console.log(JSON.stringify({ ok: captures.length > 0 && !hasLoginPrompt, loginState, captures: captures.length, endpoints: endpointList.length, outputDir: OUTPUT_DIR }, null, 2));
}

main().catch((error) => {
  ensureOutputDirs();
  const message = error instanceof Error ? error.message : String(error);
  writeReports({ generatedAt: new Date().toISOString(), loginState: "collector_error", targetUrl: TARGET_URL, cdpEndpoint: DEFAULT_CDP, captures: [], endpoints: new Map(), notes: [message] });
