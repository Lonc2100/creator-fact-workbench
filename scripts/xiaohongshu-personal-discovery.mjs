...
const TARGET_URL = "https://creator.xiaohongshu.com/";
const DEFAULT_CDP = "http://127.0.0.1:9222";
const OUTPUT_DIR = path.join(process.cwd(), ".local", "xiaohongshu-personal-v0");
const RAW_DIR = path.join(OUTPUT_DIR, "raw");
const SENSITIVE_KEY =
  /(cookie|token|csrf|xsrf|session|secret|password|passwd|authorization|auth|ticket|captcha|verify|passport|mobile|phone|email|openid|sec_uid|user_id|uid|avatar|nickname|realname|signature|sign|xsec|x-s|x-t|web_session|a1)/i;
const LONG_SECRET = /^[A-Za-z0-9+/=_-]{80,}$/;

const FIELD_TARGETS = [
  { key: "accountOverview", label: "账号总览", pattern: /(overview|summary|account|author|profile|user|dashboard|core|home|creator|作者|账号)/i },
  { key: "worksList", label: "笔记 / 作品列表", pattern: /(note|notes|item|items|post|posts|work|works|content|material|list|笔记|作品|内容)/i },
  { key: "views", label: "浏览 / 曝光", pattern: /(view|views|read|reads|show|shows|impression|exposure|pv|uv|浏览|阅读|曝光)/i },
  { key: "likes", label: "点赞", pattern: /(like|likes|liked|点赞)/i },
  { key: "comments", label: "评论数", pattern: /(comment|comments|reply|replies|评论|回复)/i },
  { key: "saves", label: "收藏", pattern: /(collect|collections|collected|favorite|favorites|save|saves|收藏)/i },
  { key: "shares", label: "分享", pattern: /(share|shares|forward|repost|分享|转发)/i },
  { key: "followersDelta", label: "粉丝变化", pattern: /(fans|fan|follower|followers|follow|new_fans|increase|delta|粉丝|关注|增量)/i },
  { key: "commentContent", label: "评论内容", pattern: /(comment.*content|comment.*text|reply.*content|reply.*text|content|text|评论内容)/i }
];

function parseArgs(argv) {
  const options = {
    target: TARGET_URL,
    cdp: process.env.XIAOHONGSHU_CDP_ENDPOINT ?? DEFAULT_CDP,
    durationMs: Number(process.env.XIAOHONGSHU_DISCOVERY_DURATION_MS ?? 60000),
    maxCaptures: Number(process.env.XIAOHONGSHU_DISCOVERY_MAX_CAPTURES ?? 80),
    maxArrayItems: Number(process.env.XIAOHONGSHU_DISCOVERY_MAX_ARRAY_ITEMS ?? 80),
    noLaunch: false
  };
...
async function launchLocalBrowser(options) {
  ...
  if (!chromePath) return { launched: false, reason: "No Chrome or Edge executable found. Set XIAOHONGSHU_CHROME_PATH or start Chrome with remote debugging." };
...
function isXiaohongshuUrl(value) {
  try {
    ...
    return hostname === "creator.xiaohongshu.com" || hostname.endsWith(".xiaohongshu.com") || hostname.endsWith(".xhscdn.com") || hostname.endsWith(".xhslink.com");
  } catch {
    return false;
  }
}
...
async function inferLoginState(page, captureCount) {
  try {
    ...
    if (/login|passport|sso/i.test(currentUrl) || /登录|扫码登录|请先登录|验证码|安全验证/.test(bodyText)) return captureCount > 0 ? "maybe_logged_in_with_login_prompt" : "needs_login";
...
function writeFieldReport({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints, notes }) {
  ...
  const lines = [
    "# Xiaohongshu Personal V0 Field Discovery Report",
    "",
    ...
    "- Raw captures are sanitized and saved only under `.local/xiaohongshu-personal-v0/raw/`.",
    "- This collector only observes the user's own logged-in Xiaohongshu creator center pages.",
  ...
  if (endpoints.length === 0) {
    lines.push("- No Xiaohongshu JSON endpoints captured.");
  } else {
...
async function main() {
  ...
  const attachPage = (page) => {
    ...
    page.on("response", async (response) => {
      try {
        ...
        if (!/json/i.test(contentType) && !/\/(api|web_api|edith|creator|data|note|notes|dashboard|operate)\//i.test(url)) return;
  ...
  let page = context.pages().find((item) => item.url().includes("xiaohongshu.com")) ?? context.pages()[0];
  if (!page) page = await context.newPage();
  attachPage(page);
  console.log("Xiaohongshu discovery collector is running. If the page asks for login or verification, complete it manually in the browser. The script will not bypass challenges.");
  ...
  if (captures.length === 0) notes.push("No Xiaohongshu JSON responses were captured during the discovery window. Try logging in, refreshing the creator center, or clicking data/notes/fans/comment modules while the collector is running.");
  const endpointList = writeEndpoints(endpoints);
