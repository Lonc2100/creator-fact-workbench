...
const OUTPUT_DIR = path.join(process.cwd(), ".local", "video-account-personal-v0");
const RAW_DIR = path.join(OUTPUT_DIR, "raw");
const SENSITIVE_KEY =
  /(cookie|token|csrf|xsrf|session|secret|password|passwd|authorization|auth|ticket|captcha|verify|passport|mobile|phone|email|openid|open_id|unionid|uin|wxuin|wxid|user_id|userid|uid|avatar|headimg|nickname|realname|signature|sign|qrcode|login|acct|bizuin|finderusername|finder_user_name|username|encrypt|key)/i;
const LONG_SECRET = /^[A-Za-z0-9+/=_-]{80,}$/;

const FIELD_TARGETS = [
  { key: "accountOverview", label: "账号总览", pattern: /(overview|summary|account|author|profile|user|dashboard|core|home|creator|finder|channel|stat|账号|主页|总览|概览)/i },
  { key: "worksList", label: "作品 / 视频数据列表", pattern: /(feed|feeds|post|posts|video|videos|work|works|content|material|finder|object|list|items|作品|视频|内容|发表)/i },
  { key: "views", label: "播放 / 浏览", pattern: /(play|plays|view|views|read|reads|watch|vv|pv|uv|impression|exposure|show|播放|浏览|阅读|观看|曝光)/i },
  { key: "likes", label: "点赞", pattern: /(like|likes|liked|praise|favorite_count|fav_count|点赞)/i },
  { key: "comments", label: "评论数", pattern: /(comment|comments|reply|replies|评论|回复|留言)/i },
  ...
  { key: "officialAccountReferral", label: "公众号阅读转化 / 引流点击", pattern: /(mp|biz|official|official_account|account_read|read_convert|convert|conversion|click|jump|link|traffic|lead|公众号|阅读|转化|引流|点击|跳转)/i },
  { key: "commentContent", label: "评论内容", pattern: /(comment.*content|comment.*text|reply.*content|reply.*text|content|text|评论内容|回复内容)/i }
];

function parseArgs(argv) {
  const options = {
    target: TARGET_URL,
    cdp: process.env.VIDEO_ACCOUNT_CDP_ENDPOINT ?? DEFAULT_CDP,
    durationMs: Number(process.env.VIDEO_ACCOUNT_DISCOVERY_DURATION_MS ?? 60000),
    maxCaptures: Number(process.env.VIDEO_ACCOUNT_DISCOVERY_MAX_CAPTURES ?? 80),
    maxArrayItems: Number(process.env.VIDEO_ACCOUNT_DISCOVERY_MAX_ARRAY_ITEMS ?? 80),
    noLaunch: false
  };
...
async function launchLocalBrowser(options) {
  ...
  if (!chromePath) return { launched: false, reason: "No Chrome or Edge executable found. Set VIDEO_ACCOUNT_CHROME_PATH or start Chrome with remote debugging." };
...
function isVideoAccountUrl(value) {
  try {
    ...
    return (
      ...
      hostname.endsWith(".channels.weixin.qq.com") ||
      hostname === "finder.video.qq.com" ||
      hostname.endsWith(".finder.video.qq.com") ||
      hostname === "weixin.qq.com" ||
      hostname.endsWith(".weixin.qq.com") ||
      hostname.endsWith(".wx.qq.com")
    );
  } catch {
    return false;
  }
...
async function inferLoginState(page, captureCount) {
  try {
    ...
    if (/login|passport|sso/i.test(currentUrl) || /登录|扫码登录|请先登录|验证码|安全验证|微信扫码/.test(bodyText)) {
...
function writeFieldReport({ generatedAt, loginState, targetUrl, cdpEndpoint, captures, endpoints, notes }) {
  ...
  const lines = [
    "# Video Account Personal V0 Field Discovery Report",
    "",
    ...
    "- Raw captures are sanitized and saved only under `.local/video-account-personal-v0/raw/`.",
    "- This collector only observes the user's own logged-in Video Account assistant / creator backend pages.",
  ...
  if (endpoints.length === 0) {
    lines.push("- No Video Account related JSON endpoints captured.");
  } else {
...
async function main() {
  ...
  const attachPage = (page) => {
    ...
    page.on("response", async (response) => {
      try {
        ...
        if (!/json/i.test(contentType) && !/\/(cgi-bin|web|api|finder|channels|platform|stat|data|comment|commentary|feed|post|opus)\//i.test(url)) return;
  ...
  let page = context.pages().find((item) => item.url().includes("channels.weixin.qq.com") || item.url().includes("finder.video.qq.com")) ?? context.pages()[0];
  if (!page) page = await context.newPage();
  attachPage(page);
  console.log("Video Account discovery collector is running. If the page asks for login or verification, complete it manually in the browser. The script will not bypass challenges.");
  ...
  if (captures.length === 0) notes.push("No Video Account related JSON responses were captured during the discovery window. Try logging in, refreshing the creator backend, or clicking overview/works/data/fans/comment modules while the collector is running.");
  const endpointList = writeEndpoints(endpoints);
