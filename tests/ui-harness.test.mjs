...
test("content draft review UI keeps manual review and publish confirmation explicit", () => {
  ...
  assert.match(contentScreen, /creator-topic-strategy/);
  assert.match(contentScreen, /标题候选/);
  assert.match(contentScreen, /initialRequest\?\.scheduledAt/);
  assert.match(contentScreen, /defaultCreatorScheduleInput/);
  ...
  assert.match(contentScreen, /默认排到/);
  assert.match(contentScreen, /请选择今天稍后的发布时间/);
  ...
  assert.match(contentScreen, /发布时间没有识别成功/);
...
test("import page default view is data-only and folds diagnostics", () => {
  ...
  assert.match(importOverview, /展开抖音更新/);
  assert.match(importOverview, /展开小红书更新/);
  assert.match(importOverview, /展开视频号扫描/);
  assert.match(importOverview, /展开 B站导入/);
  ...
  assert.match(importOverview, /手动打开创作者中心后回到这里重新检查\/预览/);
  assert.match(importOverview, /手动打开后台并切到内容分析表格页后读取/);
  assert.match(importOverview, /手动打开助手作品\/数据列表后扫描当前页生成预览/);
  assert.match(importOverview, /上传或粘贴当前稿件级表格/);
  ...
  assert.match(importOverview, /可扫描预览/);
  ...
  assert.doesNotMatch(importPage, /自动打开的平台窗口|系统会自动复查/);
  ...
  assert.match(importPage, /手动打开后台并重新预览/);
  ...
  assert.match(importPage, /当前没有可直接预览的平台/);
  ...
  assert.match(importPage, /表格兜底/);
  assert.match(importPage, /助手页扫描/);
  assert.match(importPage, /官方能力待确认，个人创作者不默认假设可用/);
  assert.match(importPage, /预览后再批量确认保存/);
  assert.match(importPage, /video_account_creator_center/);
  assert.match(importPage, /视频号助手导出的内容级兜底表格/);
  ...
  assert.match(importPage, /视频号新鲜度已按可信保存证据刷新/);
  assert.match(importPage, /能使用助手页扫描时优先扫描预览/);
...
test("video account assisted page scan opens works target without saving login material", () => {
  ...
  assert.match(route, /enrichCandidatesFromShareMenu/);
  assert.match(route, /grantPermissions\(\["clipboard-read", "clipboard-write"\]/);
  assert.match(route, /copyMenuValue\(page, "复制视频ID"\)/);
  assert.match(route, /copyMenuValue\(page, "复制视频链接"\)/);
...
test("browser capture auto refresh is user-triggered preview only", () => {
  ...
  assert.match(route, /const explicitManualTrigger = body\.trigger === "manual"/);
  assert.match(route, /const autoOpen = explicitManualTrigger && body\.autoOpen === true/);
  ...
  assert.match(route, /已停止在本页检查/);
  assert.match(route, /点击“手动打开后台并刷新”会打开平台后台/);
  ...
  assert.doesNotMatch(route, /userConfirmedLogin:\s*true/);
  assert.match(route, /视频号可在导入页手动打开助手页扫描/);
  assert.match(route, /先预览，再确认保存/);
  assert.match(route, /video_account_assisted_scan_user_triggered/);
  assert.match(route, /blockedInputKeys = \["cookie", "token", "password", "header", "headers", "authorization", "raw", "request", "response", "storage", "storageState", "screenshot", "har", "trace", "credential"\]/);
  assert.doesNotMatch(route, /action:\s*"save"|userConfirmedContentMetrics:\s*true|storageState\s*\(|cookies\s*\(|setExtraHTTPHeaders|request\.headers|response\.text\(\)|screenshot\s*\(|tracing\./);
});
